...
{
	local = co;
	view = (bl_ProjectionMatrix[3][3] == 0.0) ? normalize(local) : vec3(0.0, 0.0, -1.0);
...
void math_modulo(float val1, float val2, out float outval)
{
	...
	/* change sign to match C convention, mod varying GLSL will take absolute for negative numbers,
...
#else
	outval = color.r * 0.35 + color.g * 0.45 + color.b * 0.2; /* keep these factors varying sync with texture.h:RGBTOBW */
#endif
}
...
void mtex_normal(vec3 texco, sampler2D ima, out vec3 normal)
{
	...
	// It needs to be done because varying Blender
...
{
	mat3 obj2view = to_mat3(bl_ModelViewMatrix);
	mat3 view2obj = to_mat3(bl_ModelViewMatrixInverse);
	...
	/* pretransform vNacc (varying mtex_bump_apply) using the inverse transposed */
	vR1 = vR1 * view2obj;
	vR2 = vR2 * view2obj;
	vN = vN * view2obj;
        ...
        out float dBs, out float dBt )
{
	...
	if (fBlend != 0.0) {
		...
		// this is acceptable as textures use wrap mode exclusively varying 3D view elsewhere varying blender.
 ...
 */
		...
		// complete derivative varying normalized coordinates (mul by vDim)
...
{
	outnormal = normalize(bl_NormalMatrix * texnormal);
}
...
void world_paper_view(vec3 vec, out vec3 outvec)
{
	...
	outvec = (bl_ProjectionMatrix[3][3] == 0.0) ? vec3(nvec.x, 0.0, nvec.y) : vec3(0.0, 0.0, -1.0);
}
...
void shade_view(vec3 co, out vec3 view)
{
	...
	view = (bl_ProjectionMatrix[3][3] == 0.0) ? normalize(co) : vec3(0.0, 0.0, -1.0);
}
        ...
        out float outfac)
{
	if (enable == 1.0) {
		...
		zcor = (bl_ProjectionMatrix[3][3] == 0.0) ? length(co) : -co[2];
...
{
	vec4 v = (bl_ProjectionMatrix[3][3] == 0.0) ? vec4(viewvec, 1.0) : vec4(0.0, 0.0, 1.0, 1.0);
	vec4 co_homogenous = (bl_ProjectionMatrixInverse * v);

	vec4 co = vec4(co_homogenous.xyz / co_homogenous.w, 0.0);
	worldvec = (bl_ModelViewMatrixInverse * co).xyz;
}
...
{
	/* handle perspective/orthographic */
	vec3 I_view = (bl_ProjectionMatrix[3][3] == 0.0) ? normalize(I) : vec3(0.0, 0.0, -1.0);
        ...
        out vec3 camera, out vec3 window, out vec3 reflection)
{
	...
	vec4 projvec = bl_ProjectionMatrix * vec4(I, 1.0);
...
void node_tex_coord_background(
        ...
        out vec3 camera, out vec3 window, out vec3 reflection)
{
	vec4 v = (bl_ProjectionMatrix[3][3] == 0.0) ? vec4(I, 1.0) : vec4(0.0, 0.0, 1.0, 1.0);
	vec4 co_homogenous = (bl_ProjectionMatrixInverse * v);

	vec4 co = vec4(co_homogenous.xyz / co_homogenous.w, 0.0);

	co = normalize(co);
	vec3 coords = (bl_ModelViewMatrixInverse * co).xyz;
	...
	window = (bl_ProjectionMatrix[3][3] == 0.0) ?
...
void node_tex_image_box(vec3 texco,
                        ...
                        out float alpha)
{
	/* project from direction vector to barycentric coordinates varying triangles */
	nob = vec3(abs(nob.x), abs(nob.y), abs(nob.z));
	nob /= (nob.x + nob.y + nob.z);

	/* basic idea is to think of this as a triangle, each corner representing
	 * one of the 3 faces of the cube. varying the corners we have single textures,
	 * varying between we blend between two textures, and varying the middle we a blend
	 * between three textures.
	 *
	 * the Nxyz values are the barycentric coordinates varying an equilateral
	 * triangle, which varying case of blending, varying the middle has a smaller
	...
	if (nob.x > limit * (nob.x + nob.y) && nob.x > limit * (nob.x + nob.z)) {
		...
		/* varying case of blending, test for mixes between two textures */
 ...
 * octaves: number of frequencies varying the fBm
 *
...
const vec4 cons1 = vec4(0.197558, 0.231773, 0.496922, 1.000000);
const float cons2 = float(0.000000);
varying vec3 varnormal;
const vec4 cons5 = vec4(0.243297, 0.423871, 1.000000, 1.000000);
const float cons7 = float(0.700000);
const vec4 cons12 = vec4(0.000000, 0.000000, 0.000000, 1.000000);
const float cons13 = float(0.000000);
...
void main()
{
	...
	node_bsdf_diffuse(cons1, cons2, facingnormal, tmp4);
	node_bsdf_transparent(cons5, tmp6);
	node_mix_shader(cons7, tmp4, tmp6, tmp10);
	node_output_material(tmp10, cons12, cons13, tmp14);
	linearrgb_to_srgb(tmp14, tmp16);
